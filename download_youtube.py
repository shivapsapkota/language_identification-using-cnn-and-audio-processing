...
def download(language, source, source_name, source_type):
    ...
    if source_type == "playlist" or not os.path.exists(output_path_segmented):
        ...
        for f in files:
            ...
            command = ["ffmpeg", "-y", "-i", f, "-map", "0", "-ac", "1", "-ar", "16000", "-f", "segment", "-segment_time", "5", output_filename]
...
def download_url(language, url):
    download(language, url, url.replace("\\","_").replace("/","_").replace("@","_").replace(":","_"), "url")
...
if __name__ == '__main__':
    ...
    for language, categories in sources.items():
        ...
        for url in categories["urls"]:
            if url is None:
                continue
            download_url(language, url)
